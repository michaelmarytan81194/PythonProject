...
/****** Object:  StoredProcedure [dbo].[AD_TOP_RECRUIT_QUARTERLY]    Script Date: 01-Apr-2024 2:36:05 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE     PROCEDURE [dbo].[AD_TOP_RECRUIT_QUARTERLY] (@varREPORT_MONTH nvarchar(4000)) 
AS 
BEGIN
...
PREMIUM_CO AS (
	SELECT *
	FROM [DP_Tan].[dbo].[T_DP_TAGENTPREMIUM_TEST_CUTOFF]
	WHERE [CUTOFF] = FORMAT(EOMONTH(@varREPORT_MONTH), 'yyyyMMdd')
)
, AGT_INFO_CO AS (
	SELECT *
	FROM [DP_Tan].[dbo].[T_Main_AGENT_INFO_DA_CUTOFF]
	WHERE [CUTOFF] = FORMAT(EOMONTH(@varREPORT_MONTH), 'yyyyMMdd')
)
, DAILYSALES_FULL AS (
	SELECT 

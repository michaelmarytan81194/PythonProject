...
/****** Object:  StoredProcedure [dbo].[ACHIEVE_FYP_K2_BY_MONTHLY]    Script Date: 01-Apr-2024 2:31:13 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE     PROCEDURE [dbo].[ACHIEVE_FYP_K2_BY_MONTHLY] (@varREPORT_MONTH nvarchar(4000)) 
AS 
BEGIN
WITH
---PREPARING DATA
...
, B02 AS (
	...
	ON B021.Sales_Unit_Code collate SQL_Latin1_General_CP1_CI_AS = B022.AD_CODE collate SQL_Latin1_General_CP1_CI_AS AND B021.[Servicing Agent] = B022.[Servicing Agent]
)
, B2 AS (
	SELECT
		Sales_Unit_Code,
...
, C AS (
	...
	FROM K2_CUTOFF
	WHERE [COMPONENT STATUS] IN ('IF','LA') -- TÂN S?A 03/07/2023
	GROUP BY [AGENT NO]
)
, C1 AS (
	SELECT

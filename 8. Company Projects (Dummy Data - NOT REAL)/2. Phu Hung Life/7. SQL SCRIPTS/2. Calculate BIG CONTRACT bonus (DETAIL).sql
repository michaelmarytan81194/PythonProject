...
/****** Object:  StoredProcedure [dbo].[AD_BIGCONTRACT_BUDGET_DETAIL]    Script Date: 01-Apr-2024 2:33:31 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE   PROCEDURE [dbo].[AD_BIGCONTRACT_BUDGET_DETAIL] (@varREPORT_MONTH nvarchar(4000)) 
...
, AD_STRUCTURE AS (
	...
	ON AD_STRUCTURE1.[Territory_Code] COLLATE SQL_Latin1_General_CP1_CI_AS = AD_STRUCTURE2.[CODE] COLLATE SQL_Latin1_General_CP1_CI_AS
)
, AGENCY_STRUCTURE AS (
	SELECT
		*
...
, DAILYSALES_FULL AS (
	SELECT 
      ...
      ,[CUTOFF]
	  ,[ACK_DATE]
	FROM [DP_Tan].[dbo].[T_DP_TDAILYSALES_DA_CUTOFF]
	WHERE [CUTOFF] = FORMAT(EOMONTH(@varREPORT_MONTH), 'yyyyMMdd')
	UNION ALL
	SELECT
		[Agent Code]
      ...
      ,[CUTOFF]
	  ,[ACK_DATE]
	FROM [DP_Tan].[dbo].[T_DP_DA_Daily_DC_PO_WD_NT_CUTOFF]
	WHERE [CUTOFF] = FORMAT(EOMONTH(@varREPORT_MONTH), 'yyyyMMdd')
)
, CUSTOMER_INFO_CO AS (
	SELECT *
...
--, E AS (
	SELECT
		...
		E01.Agent_Name,
		E01.Grade,
		CAST(E01.Appointed_TAPSU AS DATE) AS Appointed_TAPSU,
		--SFC,
		E01.Policy_Number,
		E01.[Contract Type],
		CAST(E01.Proposal_Receive_Date AS DATE) AS Proposal_Receive_Date,
		CAST(E01.Policy_Issue_Date AS DATE) AS Policy_Issue_Date,
		ROUND(CAST(E01.IP_PREMIUM AS INT), 0) AS IP_PREMIUM,
		ROUND(CAST(E01.IP_COUNT_BONUS_FINAL AS INT), 0) AS IP_COUNT_BONUS_FINAL,
		E01.BONUS,
		E01.Policy_Status,
		E01.PO_TYPE,
		E01.PO_TRANSFER_DATE,
		CAST(E01.ACK_DATE AS DATE) AS ACK_DATE,
		E01.DATES_OF_FREELOOK,
		FORMAT(CAST(@varREPORT_MONTH AS date), 'yyyyMM') AS REPORT_MONTH
	-- INTO T_BIG_CONTRACT_BUDGET_DETAIL
	...
	AND E03.[AD_Grade] LIKE '%ZD%'
	AND E02.[Agent_Status] <> 'Terminated'
--)
